...
function _aws_load_sso_credentials {
    ...
    _aws_logout
    AWS_CONFIG_FILE=$(mktemp ${WSH_ROOT}/tmp/awsmfaXXXX)
...
function _aws_load_credentials_from_json {
    ...
    jq '.Credentials + .roleCredentials |
        ...
        | tee /tmp/debug-buffer | wsh-json2properties >> "${AWS_CONFIG_FILE}"
...
function _aws_load_credentials_from_sso {
...
function _aws_assume_role_and_load_credentials {
    ...
    if _aws_is_authenticated ; then
      ...
      if [ -n "${aws_mfa_token}" ]; then
        _screen_note "External ID Token provided. Adding to assumed role"
        _CMD_ASSUME_ROLE_ARGS="${_CMD_ASSUME_ROLE_ARGS} --external-id ${aws_mfa_token}"
      fi

      eval "aws sts assume-role ${_CMD_ASSUME_ROLE_ARGS}" | tee "${AWS_CONFIG_FILE}"
...
function _aws_load_mfaauth_credentials {
    ...
    ${WSH_ROOT}/bin/subcommands/wsh-token-mfaauth-create \
        "$aws_access_key_id" \
...
function _aws_load_krb5formauth_credentials {
    ...
    ${WSH_ROOT}/bin/subcommands/wsh-token-krb5formauth-create \
        --region "${region}" \
        --idp_url "${aws_idp_url}" \
...
function _aws_load_googleauth_credentials {
    ...
    AWS_CONFIG_FILE=$(mktemp ${WSH_ROOT}/tmp/awsgoogleXXXX)
...
function _aws_login {
    ...
    if [ -z $aws_id_name ]; then
        ...
        local personal_id_names="$(find -L ~/.wsh/identities/* -maxdepth 1 -type d -print 2> /dev/null | _system_xargs basename)"
        ...
        local vs_ids="$(find -L ~/.wsh/identities/* -maxdepth 1 -type d -print 2> /dev/null) $(find ~/.cloudbuilder/identities/* -maxdepth 1 -type d -print 2> /dev/null)"
        ...
        if [ ! ${#options[@]} -ge 1 ]; then
            _log "$LINENO" "No identities currently configured. Use 'wsh identity-create' to create an identity."
            return 1
        fi
        ...
        identity_path="${HOME}/.wsh/identities/${aws_id_name}"
...
function _aws_region {
    ...
    if _aws_is_authenticated ; then
        if [ -z $new_aws_region ]; then
            ...
            echo ""
            echo "Switch region with 'wsh region <name>'"
        else
            AWS_DEFAULT_REGION="${new_aws_region}"
            export AWS_DEFAULT_REGION

            _screen_note "AWS_DEFAULT_REGION now ${AWS_DEFAULT_REGION}"
        fi
    else
        _screen_error 'This command requires an active AWS session. Login first please!'
    fi
...
function _aws_logout {
    ...
    env \
        | grep '^AWS_' | grep -v 'AWS_SSO_CONFIG' \
        | awk -F'=' '{print $1}' \
...
function _aws_session_save {
    ...
    _screen_note 'AWS credentials saved to cache'
}
...
function _aws_session_load {
    ...
    if [[ -f "${CREDENTIALS_CACHE}" ]]; then
        ...
        _screen_note 'AWS credentials loaded from cache'
    else
...
function _aws_get_console_presigned_url {
    if _aws_is_authenticated ; then
      ${WSH_ROOT}/bin/tools/wsh-aws-console --stdout
    else
        _screen_error 'This command requires an active AWS session. Login first please!'
    fi
}
